...
const popoverBroker = (
  <Popover id="popover-basic" className='ef-popover'>
    <Popover.Header as="h3"><strong>INCLUIR BROKER</strong></Popover.Header>
    <Popover.Body>
      Una persona conduce la producción siguiendo un guión. La mejor manera de hacer crecer tu marca!
    </Popover.Body>
  </Popover>
);
...
const popoverDron = (
  <Popover id="popover-basic">
    <Popover.Header as="h3"><strong>INCLUIR DRON</strong></Popover.Header>
    <Popover.Body>
      Planos aéreos espectaculares para tu producción.
    </Popover.Body>
  </Popover>
);
...
const popoverContent = (
  <Popover id="popover-basic">
    ...
    <Popover.Body>
      3 Reels cortos adicionales para las redes sociales
    </Popover.Body>
  </Popover>
);
...
const popoverModeling = (
  <Popover id="popover-basic">
    <Popover.Header as="h3"><strong>INCLUIR MODELADO</strong></Popover.Header>
    <Popover.Body>
      Planos de broker realizando diferentes acciones.
    </Popover.Body>
  </Popover>
);
...
function ServiceCard({service, serviceType}: CardProps) {   
  ...
  function recalcAditionalPrice(value: boolean, aditionalType: string) {
    const aditional = service.aditionals.find(el => { return el.type === aditionalType}); 
    if (!aditional) return; 
     setFinalPrice(prev => { if (prev === 0) return prev; return value ? prev + aditional.price : prev - aditional.price; }); 
  }
  ...
  const handleSelectMeters = (meters: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (value){
      ...
      if (isDronOn) {
        const aditionalDron = service.aditionals.find(el => { return el.type === ADITIONALS.DRON});
        if (aditionalDron) {
      ...
      if (isBrokerOn) {
        const aditionalBroker = service.aditionals.find(el => { return el.type === ADITIONALS.BROKER});
        if (aditionalBroker) {
      ...
      if (isModelingOn) {
        const aditionalBroker = service.aditionals.find(el => { return el.type === ADITIONALS.MODEL});
        if (aditionalBroker) {
          finalPrice += aditionalBroker.price;
        }
      }
      ...
      if (isContentOn) {
        const aditionalBroker = service.aditionals.find(el => { return el.type === ADITIONALS.CONTENT});
        if (aditionalBroker) {
          finalPrice += aditionalBroker.price;
        }
      }
      ...
      <div className={`card-value highlighted${service.isHighlighted ? 'highlighted' : ''}`} key={service.id}>
        ...
        <div className="content">
          ...
          <div className="aditionals">
            <div className='title'>Adicionar</div>
              ...
              <div>
                ...
                <div><ToggleSwitch checked={isBrokerOn} disabled={service.group === ServiceGroup.PREMIUM} onChange={(checked) => handleToggle(checked, ADITIONALS.BROKER)}/>
                </div>
                <div className={"info-icon"}>
                  <OverlayTrigger trigger={['hover', 'focus', 'click']}  placement="top" overlay={popoverBroker}>
              ...
              <div>
                <div className='desc'>Modelado
                </div>
                <div><ToggleSwitch checked={isModelingOn} disabled={service.group === ServiceGroup.PREMIUM} onChange={(checked)=> {handleToggle(checked, ADITIONALS.MODEL)}}/>
                </div>
                <div className={"info-icon"}>
                  <OverlayTrigger trigger={['hover', 'focus','click']} placement="top" overlay={popoverModeling}>
                    <FontAwesomeIcon icon={faInfoCircle}/>
                  </OverlayTrigger>
                </div>
              </div>
              ...
              <div>
                <div className='desc'>Contenido
                </div>
                <div><ToggleSwitch checked={isContentOn} onChange={(checked)=> {handleToggle(checked, ADITIONALS.CONTENT)}}/>
                </div>
                <div className={"info-icon"}>
                  <OverlayTrigger trigger={['hover', 'focus','click']} placement="top" overlay={popoverContent}>
                    <FontAwesomeIcon icon={faInfoCircle}/>
                  </OverlayTrigger>
                </div>
              </div>
              ...
              <div>
              <div className='desc'>Dron

              </div>
                <div><ToggleSwitch checked={isDronOn} disabled={service.group === ServiceGroup.PREMIUM} onChange={(checked)=> {handleToggle(checked, ADITIONALS.DRON)}}/>
                </div>
                <div className={"info-icon"}>
            ...
            <div>
                ...
                <select onChange={handleSelectMeters}>
                  ...
                  {service.prices.map((price, i) => (
                    ...
                    <option key={i} value={price.maxM2}>Hasta {price.maxM2}m2 cubiertos</option>
                    </React.Fragment>
                  ))}
                </select> 
              )}
          ...
          <div className='prices'>
            <div>
              {finalPrice === 0 && <div className='m2-leyend'>SELECCIONA METROS CUADRADOS PARA CONOCER EL VALOR</div>}
